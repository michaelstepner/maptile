...
program define _maptile_district_courts
	...
	if ("`mergedatabase'"!="") {
		novarabbrev merge 1:1 district ///
			using `"`geofolder'/district_courts_database.dta"', nogen update replace
		exit
	}
	...
	if ("`map'"!="") {
		...
		if ("`conus'"=="conus") {
			...
			* Hide AK and HI from main map
			if ("`map_restriction'"=="") local map_restriction if !inlist(district,"D AK","D HI")
			else local map_restriction `map_restriction' & !inlist(district,"D AK","D HI")
		}
		...
		spmap `spmapvar' using `"`geofolder'/district_courts_coords.dta"' `map_restriction', id(id) ///
			`clopt' ///
			`legopt' ///
