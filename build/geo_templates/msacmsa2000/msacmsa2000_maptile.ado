...
program define _maptile_msacmsa2000
	...
	if ("`mergedatabase'"!="") {
		novarabbrev merge 1:m msacmsa2000 ///
			using `"`geofolder'/msacmsa2000_database.dta"', nogen
		exit
	}
	
	if ("`stateoutline'" != "nostateoutline") {
		local borderscode `"polygon(data("`geofolder'/msacmsa2000_stateborders.dta") ocolor(gs7) osize(medthin ...))"'
	}

	if ("`conus'"=="conus") {
		drop if inlist(msacmsa2000,3320,380)  // Drop Hawaii and Alaska: http://www.census.gov/population/estimates/metro-city/99mfips.txt
	...
	if ("`stateoutline'" != "nostateoutline") & ("`conus'"=="conus") {
		local borderscode `"polygon(data("`geofolder'/msacmsa2000_stateborders_noAKHI.dta") ocolor(gs7) osize(medthin ...))"'
	}

	if ("`map'"!="") {
		spmap `spmapvar' using `"`geofolder'/msacmsa2000_coords.dta"' `map_restriction', id(_ID) ///
			`clopt' ///
			`legopt' ///
			`legopt2' /// 
