...
program define _maptile_cbsa2009
	...
	if ("`mergedatabase'"!="") {
		novarabbrev merge 1:m cbsa2009 ///
			using `"`geofolder'/cbsa2009_database.dta"', nogen
		exit
	}
	
	if ("`stateoutline'" != "nostateoutline") {
		local borderscode `"polygon(data("`geofolder'/cbsa2009_stateborders.dta") ocolor(gs7) osize(medthin ...))"'
	}
	...
	if ("`stateoutline'" != "nostateoutline") & ("`conus'"=="conus") {
		local borderscode `"polygon(data("`geofolder'/cbsa2009_stateborders_noAKHI.dta") ocolor(gs7) osize(medthin ...))"'
	}

	if ("`map'"!="") {
		spmap `spmapvar' using `"`geofolder'/cbsa2009_coords.dta"' `map_restriction', id(_ID) ///
			`clopt' ///
			`legopt' ///
			`legopt2' /// 
