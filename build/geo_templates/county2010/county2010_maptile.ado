...
program define _maptile_county2010
	...
	if ("`mergedatabase'"!="") {
		novarabbrev merge 1:m county ///
			using `"`geofolder'/county2010_database"', nogen update replace
		exit
	}
	...
	if ("`map'"!="") {
		...
		spmap `spmapvar' using `"`geofolder'/county2010_coords"' `map_restriction', id(id) ///
			`clopt' ///
			`legopt' ///
