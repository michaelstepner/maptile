*! 9mar2016, Michael Stepner, stepner@mit.edu

program define _maptile_hrr
	...
	if ("`mergedatabase'"!="") {
		novarabbrev merge 1:m hrr using `"`geofolder'/hrr_database"', nogen update replace
		exit
	}
	...
	if ("`map'"!="") {
		...
		if ("`conus'"=="conus") {
			...
			* Hide AK and HI from main map
			if ("`map_restriction'"=="") local map_restriction if !inlist(hrr,10,150)
			else local map_restriction `map_restriction' & !inlist(hrr,10,150)
		}
		...
		spmap `spmapvar' using `"`geofolder'/hrr_coords"' `map_restriction', id(_polygonid) ///
			`clopt' ///
			`legopt' ///
