...
program define _maptile_county2014
	...
	if ("`mergedatabase'"!="") {
		novarabbrev merge 1:m county ///
			using `"`geofolder'/county2014_database"', nogen update replace
		exit
	}
	...
	if ("`map'"!="") {
		...
		spmap `spmapvar' using `"`geofolder'/county2014_coords"' `map_restriction', id(id) ///
			`clopt' ///
			`legopt' ///
