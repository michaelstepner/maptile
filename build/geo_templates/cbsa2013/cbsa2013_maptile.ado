*! 5aug2016, Meru Bhanot, meru@uchicago.edu

program define _maptile_cbsa2013
	...
	if ("`mergedatabase'"!="") {
		novarabbrev merge 1:m cbsa2013 ///
			using `"`geofolder'/cbsa2013_database.dta"', nogen
		exit
	}
	
	if ("`stateoutline'" != "nostateoutline") {
		local borderscode `"polygon(data("`geofolder'/cbsa2013_stateborders.dta") ocolor(gs7) osize(medthin ...))"'
	}

	if ("`conus'"=="conus") {
		keep if !inlist(statefp,2,15) // Drop Hawaii and Alaska
	...
	if ("`stateoutline'" != "nostateoutline") & ("`conus'"=="conus") {
		local borderscode `"polygon(data("`geofolder'/cbsa2013_stateborders_noAKHI.dta") ocolor(gs7) osize(medthin ...))"'
	}

	if ("`map'"!="") {
		spmap `spmapvar' using `"`geofolder'/cbsa2013_coords.dta"' `map_restriction', id(_ID) ///
			`clopt' ///
			`legopt' ///
			`legopt2' /// 
