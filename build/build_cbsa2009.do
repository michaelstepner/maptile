...
* imports 2009 U.S. CBSA shapefile into Stata format
...
global raw "$root/raw_data/cbsa2009"
global out "$root/geo_templates/cbsa2009"
global test "$root/tests/cbsa2009"
...
reshape_us_CBSAstateoutline using "$root/temp/CBSA_stateborders_coords.dta", ///
	save_coords("$out/cbsa2009_stateborders.dta") ///
	save_shifts("$root/temp/CBSA_AlaskaHawaiiShifts.dta")
	
project, creates("$out/cbsa2009_stateborders.dta")

* Create state outline shapefile without AK or HI

use "$out/cbsa2009_stateborders.dta", clear
drop if !missing(sname)
drop sname 
save12 "${out}/cbsa2009_stateborders_noAKHI.dta", replace
project, creates("${out}/cbsa2009_stateborders_noAKHI.dta")

*** Step 3: Process CBSA shapefile

** Step 3.1: Unzip & convert shape file to dta
project, original("$raw/US_cbsa_2009.shp")
project, relies_on("$raw/US_cbsa_2009.dbf")

shp2dta using "$raw/US_cbsa_2009.shp", ///
	database("$root/temp/cbsa2009_database.dta") ///
	coordinates("$root/temp/cbsa2009_coords.dta") ///
	replace

** Step 3.2: Clean database
use "$root/temp/cbsa2009_database.dta", clear
...
rename CBSAFP cbsa2009
destring cbsa2009, replace
recast double cbsa2009

save12 "${out}/cbsa2009_database.dta", replace
project, creates("${out}/cbsa2009_database.dta")

** Step 3.3: Fix Coordinates
use "$root/temp/cbsa2009_coords.dta", clear
merge m:1 _ID using "$out/cbsa2009_database.dta", assert(1 3) keep(3) nogen
...
save12 "$out/cbsa2009_coords.dta", replace
project, creates("$out/cbsa2009_coords.dta")
...
rm "$root/temp/cbsa2009_coords.dta"
rm "$root/temp/cbsa2009_database.dta"
rmdir "$root/temp"

** Step 3.5: Reference other files using -project-
project, relies_on("$root/readme.txt")
project, relies_on("$out/cbsa2009_maptile.ado")
project, relies_on("$out/cbsa2009_maptile.smcl")


*** Test geo-specific options
use "$out/cbsa2009_database.dta", clear
rename _ID test

maptile test, geo(cbsa2009) geofolder($out) ///
	savegraph("$test/cbsa2009_noopt.png") resolution(0.25) replace
project, creates("$test/cbsa2009_noopt.png") preserve

maptile test, geo(cbsa2009) geofolder($out) ///
	conus ///
	savegraph("$test/cbsa2009_conus.png") resolution(0.25) replace
project, creates("$test/cbsa2009_conus.png") preserve

maptile test, geo(cbsa2009) geofolder($out) ///
	nostateoutline ///
	savegraph("$test/cbsa2009_nostateoutline.png") resolution(0.25) replace
project, creates("$test/cbsa2009_nostateoutline.png") preserve

maptile test, geo(cbsa2009) geofolder($out) ///
	conus nostateoutline ///
	savegraph("$test/cbsa2009_conus_nostateoutline.png") resolution(0.25) replace
project, creates("$test/cbsa2009_conus_nostateoutline.png") preserve
