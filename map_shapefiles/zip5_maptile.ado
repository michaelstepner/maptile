...
program define _maptile_zip5
	...
	if ("`map'"!="") {
		...
		if "`conus'"!="noconus" {
			spmap `var' using `"`shapefolder'/zip5_coords_clean"' if !inrange(zip3,995,999) & !inlist(zip3,967,968) `map_restriction', id(id) ///
		...
		if "`ak'"=="ak" {
			spmap `var' using `"`shapefolder'/zip5_coords_clean"' if inrange(zip3,995,999) `map_restriction', id(id) legenda(off) ///
				oc(black) os(vthin ...) ///
				clmethod(custom) ///
				clbreaks(`min' `clbreaks' `max') ///
				fcolor(`mapcolors') ndfcolor(`ndfcolor')
		...
		if "`hi'"=="hi" {
			spmap `var' using `"`shapefolder'/zip5_coords_clean"' if inlist(zip3,967,968) `map_restriction', id(id) legenda(off) ///
				oc(black) os(vthin ...) ///
				clmethod(custom) ///
				clbreaks(`min' `clbreaks' `max') ///
				fcolor(`mapcolors') ndfcolor(`ndfcolor')
